...
{
    mem FileHandle;
    Platform_OpenFile(&FileHandle, FilePath, FILE_MODE__READ);
    if(!FileHandle)
    {
        return NULL;
    }
...
{
    mem FileHandle;
    Platform_OpenFile(&FileHandle, FilePath, FILE_MODE__WRITE);
...
internal u32
File_ReadShaders(c08 *VertexShaderFilePath,
...
File_ReadAssetPack(heap *Heap)
{
    ...
    Platform_OpenFile(&FileHandle, Str_Create(&TempStr, ASSET_PACK_FILENAME, 0), FILE_MODE__READ);
    Str_Free(TempStr);
...
File_CreateAssetPack(heap *Heap)
{
    ...
    //NOTE: Write file
        ...
        Platform_OpenFile(&FileHandle, OutFileName, FILE_MODE__WRITE);
    ...
    Platform_OpenFile(&FileHandle, ASSET_PACK_FILENAME, FILE_MODE__WRITE);
