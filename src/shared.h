...
#if _MODE_DEBUG
#   define ASSERT(Condition) if(!(Condition)) Intrin_DebugBreak()
...
#define ENUM__TYPES \
    ENUM(TYPE__S08,    sizeof(s08)) \
    ENUM(TYPE__S16,    sizeof(s16)) \
    ENUM(TYPE__S32,    sizeof(s32)) \
    ENUM(TYPE__S64,    sizeof(s64)) \
    ENUM(TYPE__U08,    sizeof(u08)) \
    ENUM(TYPE__U16,    sizeof(u16)) \
    ENUM(TYPE__U32,    sizeof(u32)) \
    ENUM(TYPE__U64,    sizeof(u64)) \
    ENUM(TYPE__R32,    sizeof(r32)) \
    ENUM(TYPE__R64,    sizeof(r64)) \
    ENUM(TYPE__C08,    sizeof(c08)) \
    ENUM(TYPE__PTR,    sizeof(vptr)) \
    /*ENUM(TYPE__ARRAY,  sizeof(array))*/ \
    ENUM(TYPE__TYPES,  sizeof(types)) \
...
global s08 __TypeSizes[TYPE__COUNT] = {
    #define ENUM(Name, Value) \
        Value,
    ENUM__TYPES
    #undef ENUM
...
#define TYPE_SIZE(Type) (__TypeSizes[Type])
...
vptr memset(vptr Dest, s32 Data, u64 Size)
{
    u08 *Dest08 = (u08*)Dest;
    while(Size--)
    {
