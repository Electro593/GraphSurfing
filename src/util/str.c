...
internal u32
ChrArr_CountPrintableASCII(c08 *Arr)
{
    u32 Count = 0;
    
    while(*Arr)
    {
        if(Chr_IsPrintable(*Arr++))
        {
            ++Count;
        }
    }
...
//     u32 DestSize = SrcSize * sizeof(c161);
         ...
         wstr Addend)
{
    ...
    Mem_Cpy(WStr + StrLen - 1, Addend, AddLen * sizeof(c16));
}
         ...
         wstr Src)
{
    ...
    Mem_Cpy(Dest, Src, SrcLen * sizeof(c16));
}
         ...
         u32 EndIndex)
{
    ...
    Mem_Cpy(Dest, Src + StartIndex, SubLen * sizeof(c16));
    Dest[SubLen] = L'\0';
}
         ...
         u32 Index)
{
    ...
    Mem_Cpy(TempStr, WStr + Index, RemLen * sizeof(c16));
    Mem_Cpy(WStr + Index, Insertion, (InsLen - 1) * sizeof(c16));
    Mem_Cpy(WStr + Index + InsLen - 1, TempStr, RemLen * sizeof(c16));
             ...
             u32 Index)
{
    ...
    Mem_Cpy(WStr + Index, Replacement, (RepLen - 1) * sizeof(c16));
}
