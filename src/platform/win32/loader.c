...
Platform_InitFunctions(platform_exports *PlatformExports)
{
    ...
    c08 *CurrProcName = (c08*)PTR_JUMP(Kernel32, ExportNamePointerTable[Index]);
    s32 CompareResult = 0;
    c08 *ProcName = "GetProcAddress";
    while(TRUE)
    {
        c08 *ProcNameIter = ProcName;
        ...
        Index = (High + Low) / 2;
        CurrProcName = (c08*)PTR_JUMP(Kernel32, ExportNamePointerTable[Index]);
    }
...
// #if _MODE_DEBUG
internal void
Platform_ThrowError_DEBUG(c08 *Message)
{
...
internal void
Platform_Print_DEBUG(c08 *Message)
{
...
Platform_CreateWorkQueueEntry(platform_work_queue *WorkQueue,
                              ...
                              vptr Parameter)
{
    // u32 CreateCursor = WorkQueue->CreateCursor;
    //TODO: InterlockedCompareExchange on this
    u32 NextCreateCursor = (/*CreateCursor*/WorkQueue->CreateCursor + 1) % WorkQueue->MaxEntryCount;
    
    ASSERT(NextCreateCursor != WorkQueue->ExecuteCursor); // Looped
    
    // u32 EntryIndex = ;
    platform_work_queue_entry *WorkQueueEntry = MEM_MEMBER(WorkQueue->Entries, platform_work_queue_entry, /*EntryIndex);//*/WorkQueue->CreateCursor);
    ...
    // InterlockedIncrement((volatile s32*)&WorkQueue->CreatedEntryCount);
...
internal b08
Platform_ExecuteWorkQueueEntry(loader_thread_info *Info)
{
    u32 ExecuteCursor = Info->Queue->ExecuteCursor;
    u32 NextExecuteCursor = (ExecuteCursor + 1) % Info->Queue->MaxEntryCount;
    
    if(ExecuteCursor != Info->Queue->CreateCursor)
    {
        u32 EntryIndex = Intrin_InterlockedCompareExchange((volatile s32*)&Info->Queue->ExecuteCursor,
                                                           NextExecuteCursor, ExecuteCursor);
        
        if(EntryIndex == ExecuteCursor)
        {
            platform_work_queue_entry *WorkQueueEntry = MEM_MEMBER(Info->Queue->Entries, platform_work_queue_entry, EntryIndex);
            WorkQueueEntry->Function(Info, WorkQueueEntry->Parameter);
            Intrin_InterlockedIncrement((volatile s32*)&Info->Queue->ExecutedEntryCount);
        }
...
Platform_GetFileDataA(c08 *FileName)
{
                  ...
                  file_mode OpenMode)
{
    ...
    switch(OpenMode)
    {
        case FILE_MODE__READ:
        {
            DesiredAccess = GENERIC_READ;
            CreationDisposition = OPEN_EXISTING;
        } break;
        
        case FILE_MODE__WRITE:
        {
            DesiredAccess = GENERIC_WRITE;
            CreationDisposition = CREATE_ALWAYS;
        } break;
        
        case FILE_MODE__APPEND:
        {
            DesiredAccess = GENERIC_WRITE;
    ...
    if(Error == ERROR_PATH_NOT_FOUND)
    {
        ...
        do
        {
            ...
            if(SlashIndex == Str_Len(FileName))
                SlashIndex = Str_FindChar(FileName, '\\', NestLevel++);
                      ...
                      file_group FileGroup)
{
    ...
    if(GivenFindHandle != INVALID_HANDLE_VALUE)
    {
        ...
        *(win32_handle*)FindHandle = *(win32_handle*)(&GivenFindHandle);
        ...
        Result = Platform_OpenFile(FileHandle, FileNameFull, FILE_MODE__READ);
...
Platform_ThreadCallback(vptr Parameter)
{
    ...
    while(TRUE)
    {
        if(!Platform_ExecuteWorkQueueEntry(ThreadInfo))
        {
...
typedef struct gen_vertex
{
    u32 SuccessorCount;
    u32 MaxSuccessorCount;
    u32 *Successors;
    
    u32 PredecessorCount;
    u32 MaxPredecessorCount;
    u32 *Predecessors;
} gen_vertex;

typedef struct save_vertex
{
    u32 SuccessorCount;
    u32 PredecessorCount;
} save_vertex;
...
#define MAX_VERTICES 461419

internal void
FindLongestShortestPath(loader_thread_info *ThreadInfo,
                        parameter *Parameters)
{
    parameter *P = Parameters + ThreadInfo->Index;
    u08 *MemBase = P->MemBase + P->MemSize * ThreadInfo->Index;
    u08 *OriginalMemBase = MemBase;
    
    s32 StartIndex = Intrin_InterlockedIncrement(P->Iteration);
    
    u32 *Distances = (u32*)MemBase;
    Mem_Zero(Distances, MAX_VERTICES * sizeof(u32));
    MemBase += MAX_VERTICES * sizeof(u32);
    
    u32 SlotSize = (P->MemSize - (u32)((u64)MemBase - (u64)OriginalMemBase)) / 3;
    u32 *Slot0 = (u32*)(MemBase);
    u32 *Slot1 = (u32*)(MemBase + SlotSize);
    u32 *Slot2 = (u32*)(MemBase + SlotSize * 2);
    ...
    u32 LayerCursor, NextLayerCount;
    u32 Distance = 2;
    while(TRUE)
    {
        LayerCursor = 0;
        ...
        while(LayerCursor < LayerCount)
        {
            u32 Curr = Layer[LayerCursor++];
            vertex *Vertex = P->Vertices + Curr;
            
            for(u32 Next = 0;
                Next < Vertex->SuccessorCount;
                ++Next)
            {
                u32 Successor = Vertex->Successors[Next];
                
                if(Distances[Successor] == 0)
                {
                    Distances[Successor] = Distance;
                    NextLayer[NextLayerCount++] = Successor;
                }
            }
        }
        
        if(NextLayerCount == 0) break;
    ...
    if(P->MaxDistance < Distance)
    {
        ...
        if(P->EndIndices == NULL)
        {
            P->StartIndices = Slot2;
            P->EndIndexCounts = Slot2 + MAX_RESULTS;
            P->EndIndices = Slot2 + MAX_RESULTS*2;
        }
        
        P->MaxDistance = Distance;
        
        P->StartIndices[0] = StartIndex;
        Mem_Cpy(P->EndIndices, Layer, LayerCount * sizeof(u32));
        P->EndIndexCounts[0] = LayerCount;
        P->TotalEndIndices = LayerCount;
        P->ResultCount = 1;
    }
    else if(P->MaxDistance == Distance)
    {
        ASSERT(P->ResultCount > 0);
        ASSERT(P->ResultCount < MAX_RESULTS);
        
        P->StartIndices[P->ResultCount] = StartIndex;
        Mem_Cpy(P->EndIndices + P->TotalEndIndices, Layer, LayerCount * sizeof(u32));
        P->EndIndexCounts[P->ResultCount] = LayerCount;
        P->TotalEndIndices += LayerCount;
        P->ResultCount++;
    }
}
...
Platform_Entry(void)
{
    ...
    PlatformState->AllocationSize = MEBIBYTES(256);
    ...
    u32 HeapSize = MEBIBYTES(250);
    ASSERT(PlatformState->AllocationUsed + HeapSize <= PlatformState->AllocationSize);
    LoaderState.Heap = Heap_Create(PlatformState->AllocationBase, HeapSize);
    PlatformState->AllocationUsed += HeapSize;
    
    u32 StackSize = MEBIBYTES(2);
    Stack_Init(PlatformState->AllocationBase + PlatformState->AllocationUsed, StackSize);
    PlatformState->AllocationUsed += StackSize;
    
    u32 StrHeapSize = MEBIBYTES(2);
    Str_SetHeap(Heap_Create(PlatformState->AllocationBase + PlatformState->AllocationUsed, StrHeapSize));
    PlatformState->AllocationUsed += StrHeapSize;
    
    // LoaderState.RawInput = Heap_Allocate(LoaderState.Heap, 0);
    // LoaderState.Window = Window;
    
    u32 VertexCount = MAX_VERTICES;
    // VertexCount = 100;
    
    //NOTE: Threading
    #if 1
    loader_thread_info ThreadInfos[12];
    platform_work_queue WorkQueue = {0};
    u32 InitialCount = 0;
    u32 ThreadCount = ARRAY_COUNT(ThreadInfos);
    WorkQueue.MaxEntryCount = VertexCount;
    WorkQueue.Entries = Heap_Allocate(LoaderState.Heap, WorkQueue.MaxEntryCount * sizeof(platform_work_queue_entry));
    WorkQueue.Semaphore = Win32_CreateSemaphoreExW(NULL, InitialCount, ThreadCount, NULL, 0, SEMAPHORE_ALL_ACCESS);
        ...
        ++ThreadIndex)
    {
        ...
        win32_handle Thread = Win32_CreateThread(NULL, 0, Platform_ThreadCallback, ThreadInfo, 0, &ThreadInfo->ID);
        Win32_CloseHandle(Thread);
    }
    #endif
    ...
    vptr FileHandle;
    str FileName = Str_Create(NULL, "data\\wiki-livingpeople-names1.txt", 0);
    Platform_OpenFile(&FileHandle, FileName, FILE_MODE__READ);
    Str_Free(FileName);
    u32 Length = (u32)Platform_GetFileLength(&FileHandle);
    u08 *Names = Heap_Allocate(LoaderState.Heap, Length);
    Platform_ReadFile(Names, &FileHandle, Length, 0);
    Platform_CloseFile(&FileHandle);
    
    FileName = Str_Create(NULL, "data\\wiki-livingpeople-links2.txt", 0);
    Platform_OpenFile(&FileHandle, FileName, FILE_MODE__READ);
    Str_Free(FileName);
    Length = (u32)Platform_GetFileLength(&FileHandle);
    u08 *Links = Heap_Allocate(LoaderState.Heap, Length);
    Platform_ReadFile(Links, &FileHandle, Length, 0);
    Platform_CloseFile(&FileHandle);
    
    FileName = Str_Create(NULL, "data\\wiki-livingpeople-binary", 0);
    Platform_OpenFile(&FileHandle, FileName, FILE_MODE__WRITE);
    Str_Free(FileName);
    
    u32 VertexCount = 461419;
    gen_vertex *Vertices = Platform_AllocateMemory(VertexCount * sizeof(gen_vertex));
    for(u32 Index = 0;
        Index < VertexCount;
        Index++)
    {
        Vertices[Index].Successors = Heap_Allocate(LoaderState.Heap, sizeof(u32));
        Vertices[Index].Predecessors = Heap_Allocate(LoaderState.Heap, sizeof(u32));
        Vertices[Index].MaxSuccessorCount = 1;
        Vertices[Index].MaxPredecessorCount = 1;
    }
    ...
    while(*LC != '\0')
    {
        ...
        u32 From = Str_To_U32(LC);
        while(*LC != ' ') LC++;
        LC++;
        
        u32 To = Str_To_U32(LC);
        while(*LC != '\n') LC++;
        LC++;
        
        gen_vertex *Vertex = Vertices + From;
        if(Vertex->SuccessorCount + 1 > Heap_GetSize(Vertex->Successors) / sizeof(u32))
        {
            Vertex->MaxSuccessorCount *= 8;
            Heap_Resize(&Vertex->Successors, Vertex->MaxSuccessorCount * sizeof(u32));
        }
        Vertex->Successors[Vertex->SuccessorCount++] = To;
        
        Vertex = Vertices + To;
        if(Vertex->PredecessorCount + 1 > Heap_GetSize(Vertex->Predecessors) / sizeof(u32))
        {
            Vertex->MaxPredecessorCount *= 8;
            Heap_Resize(&Vertex->Predecessors, Vertex->MaxPredecessorCount * sizeof(u32));
        }
        Vertex->Predecessors[Vertex->PredecessorCount++] = From;
    ...
    for(u32 VertexIndex = 0;
        ...
        ++VertexIndex)
    {
        save_vertex Vertex = {0};
        Vertex.SuccessorCount = Vertices[VertexIndex].SuccessorCount;
        Vertex.PredecessorCount = Vertices[VertexIndex].PredecessorCount;
        Platform_WriteFile(&Vertex, &FileHandle, sizeof(vertex), WriteOffset);
        WriteOffset += sizeof(save_vertex);
        
        Platform_WriteFile(Vertices[VertexIndex].Successors, &FileHandle, Vertex.SuccessorCount * sizeof(u32), WriteOffset);
        WriteOffset += Vertex.SuccessorCount * sizeof(u32);
        
        Platform_WriteFile(Vertices[VertexIndex].Predecessors, &FileHandle, Vertex.PredecessorCount * sizeof(u32), WriteOffset);
        WriteOffset += Vertex.PredecessorCount * sizeof(u32);
    }
    ...
    vptr FileHandle;
    str FileName = Str_Create(NULL, "data\\wiki-livingpeople-binary", 0);
    Platform_OpenFile(&FileHandle, FileName, FILE_MODE__READ);
    Str_Free(FileName);
    u32 Length = (u32)Platform_GetFileLength(&FileHandle);
    u08 *Data = Heap_Allocate(LoaderState.Heap, Length);
    Platform_ReadFile(Data, &FileHandle, Length, 0);
    Platform_CloseFile(&FileHandle);
    
    vertex *Vertices = Heap_Allocate(LoaderState.Heap, VertexCount * sizeof(vertex));
    u32 *Cursor = (u32*)Data;
    for(u32 Index = 0;
        Index < VertexCount;
        ++Index)
    {
        Vertices[Index].SuccessorCount = *Cursor++;
        Vertices[Index].PredecessorCount = *Cursor++;
        
        Vertices[Index].Successors = Cursor;
        for(u32 I = 0; I < Vertices[Index].SuccessorCount; ++I) Cursor++;
        
        Vertices[Index].Predecessors = Cursor;
        for(u32 I = 0; I < Vertices[Index].PredecessorCount; ++I) Cursor++;
    }
    
    volatile s32 Iteration = 6000;
    u32 MaxIterations = VertexCount - Iteration;
    // u32 MaxIterations = 1000;
    
    u32 MemSize = MEBIBYTES(8);
    parameter *Parameters = Heap_AllocateZeroed(LoaderState.Heap, sizeof(parameter) * ThreadCount);
    u08 *Mem = Platform_AllocateMemory(MemSize * ThreadCount);
    for(u32 Index = 0;
        Index < ThreadCount;
        ++Index)
    {
        Parameters[Index].MemBase = Mem;
        Parameters[Index].MemSize = MemSize;
        Parameters[Index].Vertices = Vertices;
        Parameters[Index].Iteration = &Iteration;
    }
    ...
    for(u32 Index = 0;
        ...
        ++Index)
    {
        Platform_CreateWorkQueueEntry(&WorkQueue, FindLongestShortestPath, Parameters);
    }
    
    win32_large_integer StartTime;
    Win32_QueryPerformanceCounter(&StartTime);
    
    str Temp;
    while(WorkQueue.ExecutedEntryCount != MaxIterations)
    {
        str Buffer = Str_Create(NULL, "Executed:  entries\n", 0);
        Str_Ins(&Buffer, U32_To_Str(&Temp, WorkQueue.ExecutedEntryCount), 10); Str_Free(Temp);
        Win32_OutputDebugStringA(Buffer);
        Str_Free(Buffer);
        Win32_Sleep(200);
    }
    
    win32_large_integer EndTime;
    Win32_QueryPerformanceCounter(&EndTime);
    u32 ElapsedTime = (u32)(EndTime.LowPart - StartTime.LowPart);
    ...
    for(u32 Index = 0;
        ...
        ++Index)
    {
        if(MaxDistance < Parameters[Index].MaxDistance)
        {
            MaxDistance = Parameters[Index].MaxDistance;
        }
    }
    
    FileName = Str_Create(NULL, "data\\wiki-livingpeople-results.txt", 0);
    Platform_OpenFile(&FileHandle, FileName, FILE_MODE__WRITE);
    Str_Free(FileName);
    
    u32 WriteOffset = 0;
    str Temp2 = Str_Create(NULL, "Distance: \nElapsed: \n", 0);
    Str_Ins(&Temp2, U32_To_Str(&Temp, ElapsedTime), 20); Str_Free(Temp);
    Str_Ins(&Temp2, U32_To_Str(&Temp, MaxDistance), 10); Str_Free(Temp);
    Platform_WriteFile(Temp2, &FileHandle, Str_Len(Temp2), 0);
    WriteOffset += Str_Len(Temp2);
    Str_Free(Temp2);
        ...
        ++Index)
    {
        ...
        if(P->MaxDistance == MaxDistance)
        {
            u32 EndIndex = 0;
            
            for(u32 ResultIndex = 0;
                ResultIndex < P->ResultCount;
                ++ResultIndex)
            {
                str Buffer = Str_Create(NULL, ": [", 0);
                Str_Ins(&Buffer, U32_To_Str(&Temp, P->StartIndices[ResultIndex]), 0); Str_Free(Temp);
                
                u32 MaxIndex = EndIndex + P->EndIndexCounts[ResultIndex];
                    ...
                    ++EndIndex)
                {
                    ...
                    Str_Cat(&Buffer, U32_To_Str(&Temp, P->EndIndices[EndIndex])); Str_Free(Temp);
                }
                
                Str_CatC(&Buffer, "]\n");
                
                Platform_WriteFile(Buffer, &FileHandle, Str_Len(Buffer), WriteOffset);
                WriteOffset += Str_Len(Buffer);
                Str_Free(Buffer);
            }
        }
    }
    ...
    Platform_CloseFile(&FileHandle);
    Win32_OutputDebugStringA("Ending\n");
    Win32_ExitProcess(0);
    Heap_Free(Data);
    Heap_Free(Vertices);
